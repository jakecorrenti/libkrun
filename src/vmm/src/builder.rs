...
#[cfg(target_arch = "x86_64")]
use devices::legacy::{IoApic, KvmIoapic};
use devices::legacy::{IrqChip, IrqChipDevice, IrqChipT};
#[cfg(feature = "net")]
...
pub fn build_microvm(
    ...
    #[cfg(target_arch = "x86_64")] _map_sender: crossbeam_channel::Sender<(
    ...
    #[cfg(target_arch = "x86_64")]
    {
        let ioapic: Box<dyn IrqChipT> = if vm_resources.split_irqchip {
            Box::new(
                IoApic::new(vm.fd(), _map_sender).map_err(StartMicrovmError::CreateKvmIrqChip)?,
            )
        } else {
