...
impl PciBus {
    pub fn new() -> Self {
        Self {
            devices: HashMap::new(),
            // Device 0 is left empty; the host bridge is described only in ACPI,
            // so virtio functions start at device 1.
            next_device: 1,
        }
    }

    pub fn allocate_bdf(&mut self) -> PciBdf {
        // The ECAM device field is 5 bits and decode masks to 0x1f, so a device
        // number past 31 would alias onto an existing slot.
        assert!(self.next_device < 32, "PCI device numbers exhausted");
