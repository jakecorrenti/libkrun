    ...
    pub fn write_config_register(&mut self, reg_idx: usize, offset: u8, data: &[u8]) {
        ...
        if (4..10).contains(&reg_idx) {
            ...
            if data.iter().all(|&b| b == 0xff) {
                ...
                // BARs are assigned at construction and never relocated: the guest
                // reads back the fixed address, so a reassignment write is dropped
                // here and only the size-probe flag is cleared.
